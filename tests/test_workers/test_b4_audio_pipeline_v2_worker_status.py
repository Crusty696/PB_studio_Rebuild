...
def test_audio_pipeline_v2_worker_status_writes(tmp_path):
    ...
    # Vorab Checkpoints und DB für track_id 997 aufräumen
    from services.audio_pipeline import stem_cache
    try:
        stem_cache.cache_meta_path(997).unlink(missing_ok=True)
    except Exception:
        pass
    from database import nullpool_session, AnalysisStatus
    with nullpool_session() as sess:
        sess.query(AnalysisStatus).filter_by(media_type="audio", media_id=997).delete()
        sess.commit()
