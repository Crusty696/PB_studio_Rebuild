...
def test_tools_log_action_reaches_hidden_proxy_and_log_tab() -> None:
    app = _qapp()
    host = QMainWindow()
    ...
    setup_console_source = "".join(
        inspect.getsource(PanelSetupController.setup_console).split()
    )
    assert 'self.window.right_panel.addTab(console_panel,"LOG")' in setup_console_source

    def _to_log_tab() -> None:
        host._set_context_panel_visible(True)
        for index in range(host.right_panel.count()):
            if "log" in host.right_panel.tabText(index).lower():
                host.right_panel.setCurrentIndex(index)
                return

    host._btn_toggle_console.clicked.connect(_to_log_tab)
    tools_buttons = [
        button
    ...
    try:
        ...
        assert log_actions[0].isVisible() is True
        assert log_actions[0].isEnabled() is True
        assert host._btn_toggle_console.isHidden() is True
        assert host.right_panel.currentIndex() == 0

        log_actions[0].trigger()
        app.processEvents()

        assert host._btn_toggle_console.isHidden() is True
        assert host.right_panel.isVisible() is True
        assert host.right_dock.isVisible() is True
        assert host.right_panel.tabText(host.right_panel.currentIndex()) == "Log"
    finally:
        host.close()
        host.deleteLater()
        app.processEvents()
