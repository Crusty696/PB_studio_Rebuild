...
def test_tools_chat_action_reaches_hidden_proxy_and_chat_tab() -> None:
    app = _qapp()
    host = QMainWindow()
    ...
    host.right_panel.addTab(QLabel("Chat-Inhalt"), "Chat")
    host.right_panel.setCurrentIndex(0)
    ...
    setup_chat_source = "".join(
        inspect.getsource(PanelSetupController.setup_chat_dock).split()
    )
    assert 'self.window.right_panel.insertTab(0,chat_w,"CHAT")' in setup_chat_source

    def _to_chat_tab() -> None:
        host._set_context_panel_visible(True)
        for index in range(host.right_panel.count()):
            if "chat" in host.right_panel.tabText(index).lower():
                host.right_panel.setCurrentIndex(index)
                return

    host._btn_toggle_chat.clicked.connect(_to_chat_tab)
    tools_buttons = [
        button
    ...
    chat_actions = [
        ...
        if action.text() == "KI Chat anzeigen"
    ]
    ...
    try:
        ...
        assert chat_actions[0].isVisible() is True
        assert chat_actions[0].isEnabled() is True
        assert host._btn_toggle_chat.isHidden() is True
        assert host.right_panel.currentIndex() == 0

        chat_actions[0].trigger()
        app.processEvents()

        assert host._btn_toggle_chat.isHidden() is True
        assert host.right_panel.isVisible() is True
        assert host.right_dock.isVisible() is True
        assert host.right_panel.tabText(host.right_panel.currentIndex()) == "Chat"
    finally:
        host.close()
        host.deleteLater()
        app.processEvents()
