...
def _make_schema(conn) -> None:
    conn.execute(
        text(
            # `energy` = normalisierter Motion-Score; der Worker liest ihn als
            # Primaerquelle fuer die Rollen-Regeln (VectorDB-Metadaten nur als
            # Rueckfallebene, weil sie ein Embedding voraussetzen).
            "CREATE TABLE scenes ("
