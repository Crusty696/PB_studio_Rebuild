...
def test_release_evidence_matrix_matches_current_gate_blockers() -> None:
    ...
    assert payload["release_ready"] is (not open_ids)
    assert payload["status"] == ("pass" if not open_ids else "blocked")
    assert any(
...
def test_release_evidence_matrix_keeps_required_qa_sources_visible() -> None:
    ...
    assert "not-running-as-admin" in sources["clean_vm_readiness"]["blockers"]
