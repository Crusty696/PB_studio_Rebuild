...
- 2026-06-03 B-470 (severity medium, `wiki/bugs/B-470-...md`): Projekt-Lifecycle-UI-Freeze. KORRIGIERT: create laeuft async im Worker; `_validate_and_accept` trivial. Echte Befunde via perf-watchdog Sampled Stacks: Stack A (Projekt-Switch 7s) = synchroner Timeline-Szene-Teardown auf Main-Thread -> GEFIXT `c634b39` (`timeline.load_from_db` mutet Viewport), Default-Gate `2369 passed`, Live-Verify GREEN (1132 Entries, kein load_from_db/clip_items.clear-Block mehr). Stack B/C (Create 13s) = perf-watchdog-Mislabel der modalen `dlg.exec()`-Offenzeit -> Nicht-Bug. 110s/alembic-Subitem pausiert (User-Redirect). `status: fixed` Stack A nur User.
- 2026-06-03 B-471 NEU (severity high, Cluster, `wiki/bugs/B-471-...md`): User meldete Timeline breit kaputt (Luecken/Thumbnails/Optik/Zoom/Freeze). 5 Roots read-only verifiziert. Plan `docs/superpowers/plans/2026-06-03-timeline-quality-fix-plan.md` staged T1-T5. User-Freigabe: autonom bis fertig, je TDD+Gate+Live-Verify. `status: fixed` nur User. AKTIVER Arbeitsstrang. Fortschritt: T1 Thumbnails (viewport-lazy) DONE+live-GREEN (562fe38/5e95e11). T3 Zoom-Distortion (Label ItemIgnoresTransformations) DONE+live-GREEN (1324c8c). T4 Paint/Perf DONE+code-fix-pending-live-verification (BeatGridItem implementiert, Scene-Items von Tausenden auf 1 reduziert, Culling + LOD direkt im Paint; targeted Test und Default-Gate grün). Offen: T5 Optik (subjektiv, braucht User-Richtung), Thumbnail-unter-Zoom. Side-Finding: verwaiste TimelineEntries -> clip=None -> "?"-Label/keine Thumbs.
