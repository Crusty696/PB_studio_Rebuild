...
| OTK-017 | 2 | Handoff / Vault bugs | mixed | 9 Bugs 2026-06-10 user-confirmed `fixed` nach GUI-Live-Verify (Batch 1+2): B-458 (Audio 100% inkl. mood/spectral), B-459 (Speichern dirty→save), B-460 (LUFS Long-Mix kein Timeout), B-463 (Vision→Ollama caption, kein Crash), B-464 (destruktiver Intent→Confirm-Gate), B-466 (Cancel-Status erhalten), B-467 (Export N/A-Guard), B-468 (Read-Intent→summarize_project + Write-Guard), B-472 (A1-Audio-Replace kein DB-Lock). Verbleibend offen: B-465 (UX-Preblock, kein Fix), B-469 (parked-monitoring), B-470 (Stack A fixed, 110s-alembic offen). Neu entdeckt: B-490 (FK-Error store_scenes_in_db, open). | `AGENT_HANDOFF.md`; `wiki/bugs/B-458...B-472`; `wiki/bugs/B-490`; Batch 1+2 Live-Verify + Screenshots `test_reports/live_otk017_*`. | open |
