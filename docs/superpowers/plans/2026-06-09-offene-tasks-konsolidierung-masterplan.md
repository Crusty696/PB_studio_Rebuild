...
| OTK-003 | 1 | PB-STUDIO-B471-TIMELINE-USABILITY-RECOVERY-2026-06-07 | code-complete-live-pending | B-471 user review of layout recovery; real app/user workflow still not `fixed`. 2026-06-09 agent check: existing screenshot inspected; waveform/thumbnails visible, but many labels render as square glyphs; `run_pytest_schnitt.bat` passed (`27 passed`). | Registry; `wiki/bugs/B-471...`; plan mirror; `test_reports/b471_live_test55655_layout_recovery.png`; `run_pytest_schnitt.bat` | blocked-user-review |
