...
class StructureDetectionService:
    ...
    def save_to_db(self, audio_track_id: int, result: StructureResult,
        ...
        Verwendet eine **eigene, pool-lose Engine** (NullPool) um unabhaengig
        von den Connection-Pool-Connections anderer Worker zu sein.  Bei jedem
        Retry wird zusaetzlich engine.dispose() aufgerufen und der
        busy_timeout auf 30s reduziert (statt 120s), damit Retries schneller
        rotieren koennen.
        """
