...
# AUFRAEUM B1 (konservativer God-Object-Split): entkoppelte Leaf-Helper +
# Konstanten wurden nach ``services/ingest/`` verbatim ausgelagert und werden
# hier zurueck-importiert (Re-Export). Dadurch funktionieren alle bestehenden
# Importe UND Test-Patches auf ``services.ingest_service.<name>`` unveraendert.
# Kein Logik-Change. ``engine``, ``VectorDBService``, ``get_ffprobe_bin`` und
# alle davon abhaengigen bzw. per ``inspect.getsource(ingest_service)``
# geprueften Funktionen bleiben BEWUSST in diesem Modul.
from services.ingest.constants import AUDIO_EXTENSIONS, VIDEO_EXTENSIONS
from services.ingest.helpers import (
    ...
    _json_loads_safe,
    _resolve_project_id,
    _resolve_project_id_for_ingest,
)
